...
import { courseMetaData, defaultMetaData } from '../../../App'; // Import from App.js
...
const ShareButton = ({ courseData, currentUrl }) => {
  ...
  useEffect(() => {
    if (courseData) {
      ...
      createMetaTag('name', 'keywords', courseData.keywords);
      createMetaTag('name', 'author', courseData.author);

      createMetaTag('property', 'og:title', courseData.ogTitle);
      createMetaTag('property', 'og:description', courseData.ogDescription);
      createMetaTag('property', 'og:image', courseData.ogImage);
      createMetaTag('property', 'og:image:secure_url', courseData.ogImage);
      createMetaTag('property', 'og:url', courseData.ogUrl);
      createMetaTag('property', 'og:type', 'website');
      createMetaTag('property', 'og:site_name', 'Social Prachar');
      createMetaTag('property', 'og:image:width', '1200');
      createMetaTag('property', 'og:image:height', '630');
      createMetaTag('property', 'og:image:alt', `${courseData.title} - Social Prachar`);
      ...
      createMetaTag('name', 'twitter:title', courseData.twitterTitle);
      createMetaTag('name', 'twitter:description', courseData.twitterDescription);
      createMetaTag('name', 'twitter:image', courseData.ogImage);
      createMetaTag('name', 'twitter:image:alt', `${courseData.title} - Social Prachar`);
      createMetaTag('name', 'twitter:url', courseData.ogUrl);

      createMetaTag('property', 'article:author', courseData.author);
      ...
      canonical.setAttribute('href', courseData.ogUrl);
      document.head.appendChild(canonical);
    }
  }, [courseData, currentUrl]);

  const shareData = {
    title: courseData?.title || 'Social Prachar Course',
    description: courseData?.description || 'Learn with industry experts at Social Prachar',
    url: currentUrl,
    image: courseData?.ogImage || 'https://socialprachar.com/homepic.png'
  };
...
const Headerpart = () => {
  ...
  const courseData = courseMetaData[slug] || defaultMetaData;
